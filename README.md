...
| `prediction_ranges` | `2 element Array of integers`- Optional | A collection of key-value pairs, where the keys should be identical to sequence id keys and values are arrays with the start and end region you want predicted for each sequence. Sub-arrays of 2 integers mark the start and end location of the range of prediction you are interested in.| "prediction_ranges" : [<br>  {<br>   "seq1" : [0,1000] ,<br>   "seq2" : [100,110],<br>  "random_seq" : [ <br>    [0,10],<br>    [15,100] <br>   ],<br>  "enhancer" :    [210,500],<br>  "control" : [] <br> }<br>] |
| `measurements` | `Array of floats/integers/base64`- Optional | A collection of key-value pairs, where the keys should be identical to sequence id keys and values are measurements for each sequence. Measurements can be a single value for point readouts, an array for genomic tracks or base64 to encode Hi-C matrices.| "measurements" : [<br>  {<br>   "seq1" : [0,2.5,6,..] ,<br>   "seq2" : [1,0.01,5,..],<br>  "random_seq" : [100.2,3.2,0,..],<br>  "enhancer" :    [2,7.2,0.1,..],<br>  "control" : [1.2,0.7,9,..] <br> }<br>] |
