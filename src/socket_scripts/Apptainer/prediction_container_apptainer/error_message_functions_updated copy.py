...
def check_mandatory_keys(evaluator_keys, json_return_error):
    ...
    mandatory_keys = ["request", "readout", "prediction_task", "cell_types", "sequences"]
...
def check_request(request_types, json_return_error):
    ...
    if request_types not in task_options:
    ...
    if type(request_types_types) == list:
...
def check_prediction_task_mandatory_keys(prediction_types, json_return_error):
    ...
    for prediction_task in prediction_tasks:
        ...
        if not missing:
            ...
            json_return_error['bad_prediction_request'].append(("The following keys are missing from one of the prediction_tasks" + str(missing)))

    return(json_return_error)


def check_prediction_task_name(prediction_types, json_return_error):
...
def check_prediction_task_type(prediction_types, json_return_error):
    ...
    for prediction_task in prediction_tasks:
        ...
        if prediction_type['type'] in prediction_types_options or prediction_type['type'].startswith('binding_'):
            pass
        else:
            json_return_error['bad_prediction_request'].append("prediction type " + str(prediction_type['type']) + " is not recognized")
        if isinstance(prediction_type['type'], str) == True:
            pass
        else:
            json_return_error['bad_prediction_request'].append("'type' value should be a string")

        if type(prediction_type['type']) == list:
...
def check_prediction_task_cell_type(prediction_types, json_return_error):
...
def check_prediction_task_species(prediction_types, json_return_error):
...
def check_prediction_task_scale(prediction_types, json_return_error):
