...
def run_server():
    ...
    while True:
        ...
        if evaluator_json['readout'] == "point":
            ...
            try:
                client_socket.send(point_prediction.encode("utf-8")[:1024])
            except socket.error as e:
        ...
        if evaluator_json['readout'] == "track":
            ...
            try:
                client_socket.send(track_prediction.encode("utf-8")[:1024])
            except socket.error as e:
                print ("Error sending predictor_file: %s" % e)
                sys.exit(1)
...
check_mandatory_keys(evaluator_keys, prediction_types)

def check_mandatory_keys(evaluator_keys, prediction_types):
